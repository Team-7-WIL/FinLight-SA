...
public class PdfService
{
    ...
    public byte[] GenerateQuotationPdf(Quotation quotation, Business business, Customer customer)
    {
        QuestPDF.Settings.License = LicenseType.Community;
            ...
            container.Page(page =>
            {
                    ...
                    .Column(header =>
                    {
                        ...
                        header.Item().PaddingTop(10)
                            .Row(row =>
                            {
                                row.RelativeItem().Column(col =>
                                {
                                    col.Item().Text($"Quotation Number: {quotation.Number}")
                                        .FontSize(12)
                                        .SemiBold();
                                    col.Item().Text($"Issue Date: {quotation.IssueDate?.ToString("dd MMM yyyy") ?? "N/A"}");
                                    col.Item().Text($"Expiry Date: {quotation.ExpiryDate?.ToString("dd MMM yyyy") ?? "N/A"}");
                                });
                ...
                page.Content()
                    ...
                    .Table(table =>
                    {
                        ...
                        {
                            table.Cell().Element(Block).Text((quotation.Items.ToList().IndexOf(item) + 1).ToString());
                            table.Cell().Element(Block).Text(item.Description ?? "");
                            table.Cell().Element(Block).Text(item.Quantity.ToString());
                            table.Cell().Element(Block).Text($"R{item.UnitPrice:F2}");
                            table.Cell().Element(Block).Text($"{item.VatRate:P0}");
                ...
                page.Footer()
                    .Column(footer =>
                    {
                        footer.Item().PaddingTop(20)
                            .Row(row =>
                            {
                                row.RelativeItem().Column(col =>
                                {
                                    col.Item().PaddingBottom(5).Text("Notes:")
                                        .SemiBold();
                                    col.Item().Text(quotation.Notes ?? "No additional notes");
                                });
                                ...
                                row.ConstantItem(200).Column(col =>
                                {
                                    col.Item().PaddingBottom(5).Text("Subtotal:")
                                        .SemiBold();
                                    col.Item().Text($"R{quotation.Subtotal:F2}");

                                    col.Item().PaddingTop(5).Text("VAT (Total):")
                                        .SemiBold();
                                    col.Item().Text($"R{quotation.VatAmount:F2}");
                                    ...
                                    col.Item().PaddingTop(5).Text("Total:")
                                        .FontSize(14)
                                        .SemiBold();
                                    col.Item().Text($"R{quotation.Total:F2}")
                                        .FontSize(14)
                                        .SemiBold()
                                        .FontColor(Colors.Blue.Darken2);
                                });
                        ...
                        footer.Item().PaddingTop(30)
                            .Text("This quotation is valid until the expiry date indicated above.")
                            .FontSize(12)
                            .Italic()
                            .AlignCenter();
                    });
